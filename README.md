...
| 置顶隐藏标签 | 仅**置顶瞬间**在前台不展示标签；普通瞬间与标签分类列表正常展示 |
...
> 置顶瞬间的公开 API 不返回标签；普通瞬间保留 `spec.tags`，可供主题展示与筛选。
...
- `tags`：`List<MomentTagVo>` 标签分类列表（置顶瞬间自身不展示标签，但其标签仍计入统计）
